...
const EditForm: React.FC<EditFormProps> = ({ item, onSave, onCancel, templates, onAddTemplate }) => {
  ...
  const handleSelectTemplate = (template: Partial<ScheduleItem>) => {
    setEditForm(prev => ({
      ...prev,
      ...template,
      // Preserve ID and potentially time if not in template
    }));
  };
